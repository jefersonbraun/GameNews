...
function gavern_comment_template( $comment, $args, $depth ) {
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>">	
			<aside>
				<?php echo get_avatar( $comment, ($comment->comment_parent == '0') ? 48 : 48); ?>
			</aside>
				...
				<footer>
					<?php
						...
						printf( 
							...
							sprintf( 
								'posted by <span class="author">%s</span>', 
							...
							sprintf( 
								...
								sprintf( __( '%1$s %2$s', GKTPLNAME ), 
								get_comment_date('l, d F Y'), 
								get_comment_time() )
							)
						);
					?>
...
/**
 *
 * Function used to generate post excerpt
 *
 * @return HTML output
 *
 **/
 
function gk_the_excerpt() {
	the_excerpt();
	
	echo '<a href="'.get_permalink().'" class="btn">'.__('Read more', GKTPLNAME).'</a>';
}

function gk_excerpt_more( $more ) {
	return '&hellip;';
}
	...
	if($wp_query->max_num_pages > 1) : ?>
		...
		<nav class="pagenav"<?php if($id != '') : ?> id="<?php echo $id; ?>"<?php endif; ?>>
			<ol>
				...
				<li><?php next_posts_link( __( 'Older posts', GKTPLNAME ) ); ?></li>
				<?php endif; ?>
				
				<?php if($paged > 1) : ?>
				<li><?php previous_posts_link( __( 'Newer posts', GKTPLNAME ) ); ?></li>
				<?php endif; ?>
				
				<li class="counter">
					<?php echo sprintf(__('Page %1$s of %2$s', GKTPLNAME), $paged, $wp_query->max_num_pages); ?>
				</li>
			</ol>
		</nav>
